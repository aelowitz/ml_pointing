...
def get_thermolin_correction_now(base_col_x, base_col_y, 
    ...
    start_time = core.G3Time().Now() - 10.*core.G3Units.minutes
...
def get_thermolin_correction(base_col_x, base_col_y, start_time_):
    ...
    rf_x = pickle.load(open(prefit_model_path+'/thermolin_model_xoff.pkl','r'))
    rf_y = pickle.load(open(prefit_model_path+'/thermolin_model_yoff.pkl','r'))
...
def get_thermolin_correction2(dates):
    '''
    This function calculates the correction to the collimation
    terms in the online model based on thermometry and linear
    sensor data ("thermolin" data) from "now", i.e. some number of 
    ...
    dates: array of times over which to calculate corrections to az/el.
    ...
    thermolin = read_thermolin_data_from_archive(start_time, stop_time)

    # Load the pre-fit models .
    rf = pickle.load(open('/data/jhenning/EHT_pointing/thermolin_model.pkl','r'))

    # Pass the thermolin data into the model, get the corrections.
    # Make sure all issues of cos(dec) and sign are sorted out.
    azEl_prediction = rf.predict(thermolin)[0]

    return azEl_prediction[0], azEl_prediction[1]
...
def read_thermolin_data_from_archive_3G(start_time, stop_time, arcdir='/buffer/arc/'):
